...
const Navbar = ({ user = {}, onLogout }) => {
    ...
    return (
        <header className=' sticky top-0 z-50 bg-white/90 backdrop-blur-md shadow-sm border-b border-gray-200 font-sans'>
            <div className="grid grid-cols-[1fr_auto] lg:grid-cols-[16rem_1fr_auto] items-center h-16 px-4 lg:px-6">

                <div className="hidden lg:block" />

                <div className=' flex items-center gap-2 cursor-pointer group' onClick={() => navigate('/')}>
                    <div className=' relative w-10 h-10 flex items-center justify-center rounded-xl bg-gradient-to-br
        from-fuchsia-500 via-purple-500 to-indigo-500 shadow-lg group-hover:shadow-purple-300/50 group-hover:scale-105 transition-all duration-300'>
                    ...
                    to-indigo-500 bg-clip-text text-transparent tracking-wide'>TaskNest</span>
                </div>
                {/* Right Sidebar */}
                    ...
                    <div ref={menuref} className='relative'>
                        <button onClick={handleMenuToggle} className='flex items-center gap-2 px-3 py-2 rounded-full cursor-pointer hover:bg-purple-50 transition-colors duration-300 border border-transparent hover:border-purple-200'>
                            <div className='relative'>
                                {user.avatar ? (
                                    <img src={user.avatar} alt="User Avatar" className='w-8 h-9 rounded-full shadow-sm' />
                                ) : (
