...
const getAuthHeaders = () => {
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`${API_BASE}/${task._id}/gp`, getAuthHeaders());
      onRefresh();
    } catch (err) {
      console.error(err);
      if (err.response?.status === 401) onLogout?.();
  ...
  `${API_BASE}/${task._id}/gp`,
  payload,
  getAuthHeaders() // ✅ pass it directly
);
  ...
  const progress =
    subtasks.length > 0 ? (subtasks.filter((st) => st.completed).length / subtasks.length) * 100 : 0;
      ...
      <div className={TI_CLASSES.rightContainer}>
        <div className="relative">
          <button onClick={() => setShowMenu(!showMenu)} className={TI_CLASSES.menuButton}>
            <MoreVertical className="w-4 h-4 sm:h-5 sm:w-5" size={18} />
          </button>
